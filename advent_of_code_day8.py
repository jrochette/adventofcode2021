...
class Reading:
    def __init__(self, reading: str):
        raw_signal_patterns = reading.split("|")[0].strip()
        raw_ouputs = reading.split("|")[-1].strip()

        self.signal_patterns = raw_signal_patterns.split(" ")
        self.signal_patterns.sort(key=len)
        self.outputs = raw_ouputs.split(" ")

        self.rigth_side = self.signal_patterns[0]
        self.top = (
            self.signal_patterns[1]
            .replace(self.rigth_side[0], "")
            .replace(self.rigth_side[-1], "")
        )
        self.left_top_and_mid = (
            self.signal_patterns[2]
            .replace(self.rigth_side[0], "")
            .replace(self.rigth_side[-1], "")
        )
        self.signals = {}
    ...
    # part 1
    def count_all_one_four_seven_and_eight_in_output(self) -> int:
        return len(
            [
                output
    ...
    # part 2
    def get_parsed_output(self) -> int:
        return int(
            "".join(
                map(lambda output: self.signals["".join(sorted(output))], self.outputs)
            )
        )
    ...
    def _parse_patterns(self):
        for pattern in self.signal_patterns:
            ...
            if self._is_zero(sorted_pattern):
                self.signals[sorted_pattern] = "0"
            elif self._is_one(sorted_pattern):
                self.signals[sorted_pattern] = "1"
            elif self._is_two(sorted_pattern):
                self.signals[sorted_pattern] = "2"
            elif self._is_three(sorted_pattern):
                self.signals[sorted_pattern] = "3"
            elif self._is_four(sorted_pattern):
                self.signals[sorted_pattern] = "4"
            elif self._is_five(sorted_pattern):
                self.signals[sorted_pattern] = "5"
            elif self._is_six(sorted_pattern):
                self.signals[sorted_pattern] = "6"
            elif self._is_seven(sorted_pattern):
                self.signals[sorted_pattern] = "7"
            elif self._is_eight(sorted_pattern):
                self.signals[sorted_pattern] = "8"
            elif self._is_nine(sorted_pattern):
                self.signals[sorted_pattern] = "9"

    def _is_zero(self, output: str) -> bool:
        return (
            len(output) == 6
            and (self.rigth_side[0] in output and self.rigth_side[-1] in output)
            and not (
                self.left_top_and_mid[0] in output
                and self.left_top_and_mid[-1] in output
            )
        )

    def _is_one(self, output: str) -> bool:
        return len(output) == 2

    def _is_two(self, output: str) -> bool:
        return (
            len(output) == 5
            and not (self.rigth_side[0] in output and self.rigth_side[-1] in output)
            and not (
                self.left_top_and_mid[0] in output
                and self.left_top_and_mid[-1] in output
            )
    ...
    def _is_three(self, output: str) -> bool:
        return len(output) == 5 and (
            self.rigth_side[0] in output and self.rigth_side[-1] in output
        )

    def _is_four(self, output: str) -> bool:
        return len(output) == 4

    def _is_five(self, output: str) -> bool:
        return (
            len(output) == 5
            and not (self.rigth_side[0] in output and self.rigth_side[-1] in output)
            and (
    ...
    def _is_six(self, output: str) -> bool:
        return (
            len(output) == 6
            and not (self.rigth_side[0] in output and self.rigth_side[-1] in output)
            and (
    ...
    def _is_seven(self, output: str) -> bool:
        return len(output) == 3

    def _is_eight(self, output: str) -> bool:
        return len(output) == 7

    def _is_nine(self, output: str) -> bool:
        return (
            len(output) == 6
            and (self.rigth_side[0] in output and self.rigth_side[-1] in output)
            and (
...
readings = list(map(lambda reading: Reading(reading), original_input))

# part 1
count = 0
sum = 0
for reading in readings:
    count += reading.count_all_one_four_seven_and_eight_in_output()
    sum += reading.get_parsed_output()

print(f"Total count of 1,4,7 and 8: {count}")
print(f"Total sum of outputs: {sum}")
